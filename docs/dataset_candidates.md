...
> **硬件约束**：NVIDIA RTX 5070 Laptop GPU, 8 GB VRAM；D 盘剩余空间充足（采样时 `NORMAL`，约 79 GB 可用）。
...
> 标注缩写：Seg = 语义/实例分割（mask）；Det = 目标检测（bounding box）。
...
| **论文** | Hwang, Yang, Nguyen, et al. *GuideTWSI: A Diverse TWSI Dataset from Synthetic and Real-World Images for Blind and Low-Vision Navigation*. **ICRA 2026**, Best Paper Finalist (Field & Service Robotics). arXiv:2603.07060 |
| **官方来源** | GitHub: `https://github.com/DARoSLab/GuideTWSI` ｜ Project: `https://guidedogrobot-tactile.github.io/` ｜ Kaggle: `guidedogrobot/guidetwsi` ｜ HF: `guidedogrobot-tactile/GuideTWSI` |
...
| **标注类型** | 分割掩码（polygon / RLE）+ 2D 边界框（COCO，合成集含）+ 深度图（合成集）；**提供 YOLOv11-seg 格式（polygon-only）及 `format_converters.py` 支持 YOLO↔COCO↔SAM2↔RLE** |
| **是否包含盲道** | ✅ 同时覆盖**条状盲道(directional bars)** 与 **圆点盲道(truncated domes)**，地理多样性最强 |
| **是否包含障碍物** | ❌ 仅聚焦 TWSI 触觉地标本身，不含占用盲道的障碍物 |
| **是否 segmentation** | ✅ 主力标注为分割掩码 |
| **是否 detection** | ✅ 合成集提供 COCO bbox；论文用 YOLOv11-seg 训练检测+分割 |
| **数据大小** | 检索未给出精确体积；**下载前需用磁盘闸门确认**。依 39.5K 图（含 15K 合成 640–1280px + 真实图）粗估约 **15–30 GB**（全量）；仅取 RBar-22K 子集约 5–10 GB |
| **下载方式** | Kaggle 网页下载，或 `huggingface-cli download guidedogrobot-tactile/GuideTWSI --repo-type dataset --local-dir ./data`；预训练 YOLO 权重 `guidedogrobot-tactile/GuideTWSI-weights` |
| **是否适合 YOLO** | ✅✅ **最佳**。官方即用 Ultralytics(YOLOv11-seg) 训练并给出权重 `yolo11n_tactile.pt`；提供格式转换脚本 |

**亮点**：规模最大、最多样；合成数据增强让 dome 分割 mIoU 最高 +29 点；真实机器人实测停止成功率 96.15%。MIT 授权，可直接用于本系统。
...
| **官方来源** | GitHub: `https://github.com/kxzr/WOTR`（含 Baidu CODE / Google Drive 链接）；类别含 `tactile_paving`(→ blind_road)、`roadblock`、`warning_column`、`reflective_cone`、`pole` 等 20 类 |
| **License** | **MIT** |
| **图片数量** | **13,928** 张（train 9056 / val 2338 / test 2534），约 190K 标注框 |
| **标注类型** | **PASCAL-VOC 格式边界框**（20 类：15 障碍物 + 5 路况判断，含 `tactile_paving/blind_road`）；类映射文件 `data.yaml` 已给出，易转 YOLO txt |
| **是否包含盲道** | ✅ 专设 `tactile_paving`(盲道) 类 |
| **是否包含障碍物** | ✅✅ 15 类障碍物（树、路障、警示柱、反光锥、灰罐、行人、自行车、公交、卡车、汽车、摩托、三轮、消防栓、路障柱、垃圾箱等） |
| **是否 segmentation** | ❌ 仅检测框 |
| **是否 detection** | ✅ 边界框 |
| **数据大小** | 检索未给精确体积；13,928 张街景图粗估约 **2–4 GB**（VOC 图像为主） |
| **下载方式** | GitHub 仓库内的 Baidu CODE（提取码 WOTR）/ Google Drive 链接 |
| **是否适合 YOLO** | ✅✅ **最佳**。VOC 框直接转 YOLO；论文配套 **PC-YOLO(基于 YOLOv7)**，社区已用 YOLOv8/v11 验证 |
...
| **论文** | Takano, Nakane, Yu, Zhang. *Tactile Paving Detection and Tracking Using Tenji10K Dataset*. **IEEJ Transactions on Electrical and Electronic Engineering**, 2024. DOI: 10.1002/tee.24123 |
...
| **图片数量** | **10,000** 张（20 个序列的第一视角视频帧），日本实地采集，分辨率 640×480@30fps，智能眼镜 1.7m 高度 |
| **标注类型** | 每张图配触觉铺路**掩码，以两条边界线表示**（第一视角下盲道呈自底向上的竖向区域，故用双线刻画）；非标准多边形/框 |
...
| **官方来源** | Project: `https://ytaek-oh.github.io/sideguide`（需填问卷+同意条款后获下载链接）；亦经韩国 AI Hub 发布 |
| **License** | **申请制 / 研究用途**（要求同意 T&C、填问卷审批，非宽松开源） |
| **图片数量** | **350K** 图含 bbox 标注 + **100K** 图含 polygon mask + 180K 立体图像对（含视差真值） |
...
| **是否适合 YOLO** | △ 格式可用但**体量过大 + 许可限制**，对 8GB 显存/有限磁盘不友好；更适合作为 GuideTWSI 的已编译来源而非直接训练集 |
...
| **论文** | Zhang, Liang, Zhao, Wang. *GRFB-UNet: A new multiscale attention network... for tactile paving segmentation*. **Expert Systems with Applications**（录用） |
| **官方来源** | GitHub: `https://github.com/lmsha1/GRFB-Unet`（代码+样例）；全量数据 Baidu Netdisk（密码 9ope）/ Google Drive |
...
| **官方来源** | 论文及作者仓库（检索未定位到独立公开仓库主页；论文声明数据集公开） |
...
| 字段 | 内容 |
|------|------|
| **名称** | Obstacles in Public Spaces for Dist-YOLO |
| **论文/来源** | Kaggle 社区数据集（为 Dist-YOLO 模型标注） |
| **官方来源** | `https://www.kaggle.com/datasets/muftirestumahesa/obstacles-in-public-spaces-for-dist-yolo` |
...
- **ROD-Dataset** (HuggingFace `Abtinz/Obstacle-Detection-Dataset-YOLO`)：24,326 图 / 25 类 / **YOLO Darknet txt 原生** / CC BY 4.0。含 Person, Car, Tree, Manhole, Bench, Bicycle Rack 等街具，覆盖盲道占用场景，适合扩充障碍物类。
- **Mendeley VI Navigation Dataset** (`m68g3h7p87`)：8,114 图 / 22 类 / 实例分割 + VOC / **CC BY 4.0** / 579 MB。视角贴近视障人士，含 Obstacle、Footpath、Tree、Person 等，可作小体量补充。

---

## 三、YOLO 适用性结论

- **直接 YOLO 友好（bbox/txt 现成或可一键转）**：WOTR(VOC→YOLO)、GuideTWSI(官方 YOLO-seg + 转换器)、Obstacles in Public Spaces(原生 YOLO)、ROD(原生 YOLO)。
...
- **WOTR** 作为系统主干：它是唯一同时含「盲道类(tactile_paving/blind_road)」+「15 类障碍物」且 MIT 授权的数据集，最贴合「盲道障碍物监测与预警」目标，体量适中（约 2–4 GB），8 GB 显存可全量训练。
- **GuideTWSI** 作为盲道专精增强：提供最高质量、最多样的盲道分割/检测标注（条状+圆点、真实+合成、机器人视角），MIT 授权，官方 YOLOv11-seg 权重可直接复用或微调。其 RBar-22K 子集可显著强化盲道检测召回。

**障碍物类扩充（可选）**：Obstacles in Public Spaces (CC0, 原生 YOLO) 与 ROD-Dataset (CC BY 4.0, 原生 YOLO) 用于补齐 WOTR 未覆盖的障碍物类别（坑洼、井盖、路障等）。

### 理由

1. **License 安全**：WOTR 与 GuideTWSI 均为 **MIT**，可商用、可改、无申请门槛，避免 SideGuide(申请制)/TP-Dataset(非商业)/Tenji10K(不明) 的合规风险。
2. **任务匹配**：本项目要监测「盲道 + 占用盲道的障碍物」，WOTR 一套数据覆盖两者；GuideTWSI 补足盲道本身的精细检测/分割。
3. **YOLO 生态就绪**：两者均提供或易转 YOLO 格式，且 GuideTWSI 已有 Ultralytics 训练范式与预训练权重，Phase 09 训练可快速起步。
...
2. 标注统一：WOTR(VOC)→YOLO txt；GuideTWSI 用官方 YOLO-seg 或 `format_converters.py` 导出。
3. 类别体系设计：建议合并为「盲道 / 各类障碍物」两级，注意 WOTR 的 `tactile_paving`↔`blind_road` 与 GuideTWSI 的 tactile 类对齐。
4. 所有下载数据落入 `datasets/`（已被 `.gitignore` 屏蔽，不入库）。
