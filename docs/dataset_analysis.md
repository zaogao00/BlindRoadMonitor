...
| **最大风险** | ① 盲道类仅 2,381 实例（占总量 1.21%），样本偏少；② 划分泄漏 **15 组跨划分重复图**（共 20 组重复，见 §7.2）；③ 类别长尾 **437:1**。 |
...
| 划分可用 | ⚠️ | 划分齐全，但存在 **15 组跨划分重复**（WOTR 7 + ROD 8，共 20 组重复，见 §7.2），须去重。 |
...
| **合计** | **20 组** | 20 张 | **15 组跨 split**（WOTR 7 + ROD 8，属划分泄漏）；5 组为划分内部重复（WOTR train 内 4、ROD valid 内 1） |

- **风险**：同一张图同时出现在 train 与 test → 评估指标虚高（ memorize 而非泛化）。
- **建议策略**：按 MD5 分组，**优先保留 val/test 中的副本，从 train 中剔除重复项**（训练集精确损失 **13 张** = WOTR 9（train 内部 4 + 跨划分 5）+ ROD 4，测试集保持纯净）。
